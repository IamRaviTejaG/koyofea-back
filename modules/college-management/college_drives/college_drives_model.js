...
export let college_drives_model = {
  get_all: (college_id) => {
    let sql = `SELECT rd.id, r.name AS company_name, rd.name as drive_name,
               rd.created, rd.status, jt.name AS job_type, rdr.college_accept
               ...
               INNER
               JOIN recruiter_drive_response rdr
               ON rdr.college_id = mdc.college_id
               AND rdr.recruiter_drive_id = rd.id
